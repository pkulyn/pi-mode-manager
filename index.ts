...
const MODE_CONTEXT_TYPES: Record<Mode, string> = {
	plan: "pi-plan-mode-plan-context",
	auto: "pi-plan-mode-auto-context",
	edit: "pi-plan-mode-edit-context",
};
...
export default function modeManagerExtension(pi: ExtensionAPI): void {
	...
	function updateStatus(ctx: ExtensionContext): void {
		if (enabled) {
			ctx.ui.setStatus("pi-plan-mode", `${MODE_ANSI[mode]}${MODE_LABEL[mode]}${ANSI_RESET}`);
		} else {
			ctx.ui.setStatus("pi-plan-mode", undefined);
		}

		if (!enabled || todoItems.length === 0) {
			ctx.ui.setWidget("pi-plan-mode-todos", undefined);
			return;
		}
		...
		ctx.ui.setWidget("pi-plan-mode-todos", [heading, ...laidOutItems], {
	...
	// the plan was produced before pi-plan-mode was enabled, or the state
	...
	function enableModeManager(ctx: ExtensionContext): void {
		...
		allDoneNotified = false;
		// No history scan on enable: before pi-plan-mode is on, the model has
	...
	pi.on("session_start", async (_event, ctx) => {
		// Default: pi-plan-mode auto-starts with every pi launch in Auto mode.
		...
		const modeManagerEntry = entries
			.filter(
				(e: { type: string; customType?: string }) =>
					e.type === "custom" &&
					(e.customType === "pi-plan-mode" ||
						e.customType === "plan-mode" ||
						e.customType === "mode-manager"), // legacy keys from pre-rename
			)
			.pop() as { data?: ModeManagerState } | undefined;

		if (modeManagerEntry?.data) {
			// Restore progress and the pre-pi-plan-mode toolset. `enabled` and
		...
		// Auto mode restores the pre-pi-plan-mode toolset; capture it now so a
